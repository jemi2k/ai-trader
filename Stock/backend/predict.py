...
def predict_stock(symbol, period, sim, future):
   ...
   # for no, r in enumerate(accepted_results):
    ...
    plt.figure(figsize=(11, 5))
    for no, r in enumerate(accepted_results):
        # Slice the data using a step of 5
        x_data = date_ori[::5]
        y_data = r[::5]
        ...
        # Build the tooltip labels using the trimmed data
        labels = [
            f"<table style='border: 1px solid black; font-weight:bold; font-size:larger; background-color:white'>"
            f"<tr style='border: 1px solid black;'><th style='border: 1px solid black;'>Date:</th>"
            f"<td style='border: 1px solid black;'>{x}</td></tr>"
            f"<tr style='border: 1px solid black;'><th style='border: 1px solid black;'>Close:</th>"
            f"<td style='border: 1px solid black;'>{round(y,2)}</td></tr></table>"
            for x, y in zip(x_data, y_data)
        ]
        lines = plt.plot(x_data, y_data, label=f"forecast {no + 1}", marker="*")
